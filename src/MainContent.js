...
export default function MainContent() {
  ...
  const changeAccent = (e) =>{
    console.log(e.target.value)
    setTheme({...theme, accent:e.target.value})
  }
  return (
    <>
      <Row className={`p-4 bg-${theme.bg} text-${theme.text}`}>
        <Col lg={3} className="border-right p-2">
          <Stack gap={2}>
            <h2 className={`bg-${theme.accent} text-center text-white`}>
              First item
            </h2>
            <h2 className="text-center">Second item</h2>
            <h2 className="text-center">Third item</h2>
            <select className= {`mb-3 bg-${theme.bg} text-${theme.text}`} onChange={(e) => changeAccent(e)} value={theme.accent}>
              {["primary", "secondary", "success", "danger", "warning", "info"].map(c => {
                return (
                  <option key={`${c}-accent`} className="h4">{c}</option>
                )
              })}
            </select>
          </Stack>
        ...
        <Col>
          <div className={`h4 vh-100 overflow-scroll text-${theme.text}`}>
            ...
            or else he endures pains to avoid worse pains." so blinded by
            ...
            endures pains to avoid worse pains." so blinded by desire, that they
            ...
            worse pains." so blinded by desire, that they cannot foresee the
            pain and trouble that are bound to ensue; and equal blame belongs to
            ...
            pleasures, or else he endures pains to avoid worse pains." so
